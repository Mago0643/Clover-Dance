...
class Glitch2Handler
{
  public var shader(default,never) = new Glitch2Shader();
  public var glitch(default,set):Float = 0;
  public var iTime(default,set):Float = 0;
  function set_glitch(f) {
    glitch = f;
    shader.GLITCH.value=[f];
    return f;
  }
  function set_iTime(f) {
    iTime = f;
    shader.iTime.value=[f];
    return f;
  }
  public function new(){shader.GLITCH.value=[0];shader.iTime.value=[0];}
}

// by https://www.shadertoy.com/view/lsfGD2
class Glitch2Shader extends FlxShader
{
  @:glFragmentSource('
  #pragma header
